...
class ReporteEncontradaController extends Controller
{
    ...
    {
        $pagina = $consultar->listar(TipoReporte::Encontrada, $request->integer('page', 1));
        $reportes = $presenter->paginar($pagina, $request);

        return view('reportes.encontradas.index', compact('reportes'));
    }
    ...
    public function show(string $id, ConsultarReportes $consultar): View
    {
        ...
        $reporte = $consultar->buscar($id, TipoReporte::Encontrada);
        abort_if($reporte === null, 404);

        return view('reportes.encontradas.show', compact('reporte'));
    }
}
